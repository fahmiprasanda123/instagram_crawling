...
while True:
	...
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=280&nourut=7&engine=com")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			api = InstagramAPI("hendrasusanto212", "Ap@l@h") #pinjam like 7
			api.login()
			print('GET COMMENT ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=7&idkeyword="+str(idkeyword)+"&engine=com&sosmed=instagram")
