...
# 		user_feed.extend(api.LastJson.get('users', []))
# 		next_max_id = api.LastJson.get('next_max_id', '')
# 		print(str(i)+' pleace wait.. get nex page: '+str(next_max_id))
# 		#time.sleep(70)
# 		return user_feed
# 		if (i == 40):
# 			next_max_id = False
...
# 			next_max_id = ''
# 		_ = api.getHashtagFeed(user_id, maxid=next_max_id)
# 		user_feed.extend(api.LastJson.get('items', []))
# 		next_max_id = api.LastJson.get('next_max_id', '')
# 		print(str(i)+' pleace wait ..nex page: '+str(next_max_id))
# 		print("page:"+str(i))
# 		time.sleep(5)
# 		return user_feed
# 		if (i == 40):
...
while True:
	pass
	resp = requests.get("https://wa.dad.id/search_project?nourut=4&engine=feed")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			#api = InstagramAPI("bellarosa683", "shampobayi")-->chalange Requere
			#api = InstagramAPI("yabobay3", "sobari85")
			# api = InstagramAPI("novaltarin", "p3r3mpuan")
			# api = InstagramAPI("yabobay3", "sobari85")
			#api = InstagramAPI("yabobay3", "sobari85") #pinjam ke get_comment4
			api = InstagramAPI("testingakun82", "tes123456")
			...
			if row["type"]=='Account':
				...
				# formatted_json = pprint.pformat(user_feed)
				...
				user_feed = api.getTotalHastagFeed(keyword)
				# formatted_json = pprint.pformat(user_feed)
				# print(formatted_json)
