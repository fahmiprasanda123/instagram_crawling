...
while True:
	pass
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=1&engine=like")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			#api = InstagramAPI("ronaamiraaa", "amiraa12345")
			# api = InstagramAPI("7197.della", "dirumah")
			# api = InstagramAPI("hendrasusanto212", "Ap@l@h")
			api = InstagramAPI("lucas_renaldoo", "lucas12345")
			api.login()
			print('GET LIKE ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=1&idkeyword="+str(idkeyword)+"&engine=like&sosmed=instagram")
