...
#GE USERDATA FEED============================================================|
def getComments(api,idkeyword,alias_kyword,keywordname,idproject,projectname,categoryid,category,datafeed):
...
def save_phrase(textcomment,username,idkeyword,keywordname,idproject,projectname,code,postid,datepost):
	#try:
		print("2. GET PHRASE")
		print(textcomment)
		captiondata2=re.sub('[/".,!@$]', '',str(textcomment))
		analysis = TextBlob(captiondata2)
		...
		ss=a.tags
		phrase=a
		print(phrase.noun_phrases)
		totarr=len(phrase.noun_phrases)-1
		i=1
		for np in phrase.noun_phrases:
			try:
				if(np.detect_language()=='id'):
					parseexst=np
				else:
					parseexst=np.translate(to='id')
				#parseexst=np
				print(parseexst)
				link='https://www.instagram.com/p/'+code
				posts = {
				"trending":str(parseexst),
				"sumber":str(link),
				"datepost":str(datepost),
				...
				post_esphrase(posts,str(postid)+str(i))
				time.sleep(6)
				if (i == int(totarr)):
					update_elasticsearch(str(postid))
					break
				i+=1
			except:
				print('eror!!..Translation API returned the input string unchanged.')
				time.sleep(6)


#GE KEYWORD/ACOUNT BY PROJECT========================================================|
while True:
	pass
	#resp = requests.get("https://wa.dad.id/search_project?nourut=0&engine=com")
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=5&engine=com")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			keyword=row["keyword"]
			alias_kyword=row["alias_kyword"]
			idproject=row["idproject"]
			projectname=row["project_name"]
			categoryid=row["categoryid"]
			category=row["category"]
			#api = InstagramAPI("cintaoktarina", "bajubagus")
			#api = InstagramAPI("nouval255", "nouval123")
			api = InstagramAPI("getcom2", "tes123456") #pinjam di get_profilelike7
			api.login()
			print('GET COMMENT ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=5&idkeyword="+str(idkeyword)+"&engine=com&sosmed=instagram")
			...
			if(feed.status_code==200):
				if(int(len(jsondata['hits']['hits']))==0):
					...
					getComments(api,idkeyword,alias_kyword,keyword,idproject,projectname,categoryid,category,datafeed)
					time.sleep(100)
		else:
			posts = {
			"status_engine":int('3'),
