...
def getUserlike(api,idkeyword,keywordname,idproject,projectname,categoryid,category_project,datafeed):
	...
	for doc in datafeed:
		...
		result_procces= api.LastJson['status']
		if(result_procces=='fail'):
			print("proses tidak dilanjutakan karen fail")
		else:
			print("dijalankan")
			resultuser = api.LastJson['users']
			totalmedialike=len(resultuser)
			print(totalmedialike)
...
while True:
	pass
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=2&engine=like")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=2&idkeyword="+str(idkeyword)+"&engine=like&sosmed=instagram")
