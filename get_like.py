		...
		for item in resultuser:
			#try:
				...
				if(audience_total==0):
					...
					print('Audience already available'+' Loop:'+str(i))
				totali=len(str(i))
				totali2=totali-1
				number = str(i)
				if(totali>1):
...
while True:
	pass
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=0&engine=like")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			#api = InstagramAPI("ronaamiraaa", "amiraa12345") ---chalange requered
			api = InstagramAPI("danidanu670", "lemarikaca") #---not login
			#api = InstagramAPI("bastiangody22", "bastian12345")
			api.login()
			print('GET LIKE ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=0&idkeyword="+str(idkeyword)+"&engine=like&sosmed=instagram")
