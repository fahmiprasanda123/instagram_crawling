...
while True:
	...
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=1&engine=com")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			api = InstagramAPI("nouval255", "Anakku")
			api.login()
			print('GET COMMENT ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=1&idkeyword="+str(idkeyword)+"&engine=com&sosmed=instagram")
