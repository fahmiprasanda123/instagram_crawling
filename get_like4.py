...
while True:
	pass
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=4&engine=like")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			#api = InstagramAPI("testingakun16", "tes123456")----akun fahmi, dalam pantawan
			#sementara pakai ge_like5
			#api = InstagramAPI("mark_geraldii", "geraldi12345")
			# api = InstagramAPI("karina_sandraaa", "sandraa12345")
			# api = InstagramAPI("getprofilelike4", "tes123456")
			# api = InstagramAPI("yabobay3", "sobari85")
			api = InstagramAPI("sariningsih565", "sajadah")
			api.login()
			print('GET LIKE ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=4&idkeyword="+str(idkeyword)+"&engine=like&sosmed=instagram")
