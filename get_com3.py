...
while True:
	...
	resp = requests.get("https://wa.dad.id/search_projectlike?idproject=317&nourut=3&engine=com")
	...
	if(resp.status_code==200):
		...
		if(int(row["tot"])>0):
			...
			api = InstagramAPI("getcom3", "tes123456")
			#api = InstagramAPI("cintaoktarina", "bajubagus")
			#api = InstagramAPI("nouval255", "nouval123")
			api.login()
			print('GET COMMENT ACOUNT?KEYWORD'+ str(keyword))
			print('-------------------------------------------------------------')
			feed = requests.get("https://wa.dad.id/search_feed?nourut=3&idkeyword="+str(idkeyword)+"&engine=com&sosmed=instagram")
